...
try:
    ...
    if "Version-1.5" in response:
        ...
        if kex in response:
            ...
            linex()
            print("Mr Nigga Approve My Key")
            linex()
            print(" Your key: "+fkeyx);subprocess.check_output(["am", "start", "https://api.whatsapp.com/send?phone=+923203714588&text="+(" Hi Nigga Approve My Key\n Token:- "+ fkeyx)]);time.sleep(2)
...
def Create():
    ...
    def strt():
       try:
           ...
           import sys
           loop+=1
           sys.stdout.write('\r\r\033[1;37m [NIGGA-CREATE] \033[1;32mOK:%s \033[1;37m'%(ok));sys.stdout.flush()
           requests=r.Session()
           from fake_email import Email
           mmail=Email().Mail()
           em=mmail['mail']
           ...
           pass
    file="/sdcard/NIGGA/CREATE-OK.txt"
    u=5000
    clear()
